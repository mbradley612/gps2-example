...
/*
* Initializing the global device is all through config. The only mandatory setting is the
* UART number and baud rate. For the others you can rely on the defaults.
*/
enum mgos_app_init_result *gps2_init_global_device(void) {
...
enum mgos_app_init_result mgos_app_init(void) {
  ...
  device = gps2_create_uart(UART_NO, &ucfg, NULL, NULL);
